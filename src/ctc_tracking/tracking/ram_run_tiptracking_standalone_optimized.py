...
from ctc_tracking.tracking.temporal_downsample_ctc_results import temporal_downsample_tracked_stack
...
def _label_frame_index(mask_stack: np.ndarray) -> dict[int, list[int]]:
    frames_by_label: dict[int, list[int]] = {}
    for frame_idx in range(int(mask_stack.shape[2])):
        for track_id in _track_ids_in_frame(mask_stack[:, :, frame_idx]).tolist():
            frames_by_label.setdefault(int(track_id), []).append(frame_idx)
    return frames_by_label


def _fill_short_track_gaps(
    mask_stack: np.ndarray,
    max_gap_frames: int,
    min_fill_ratio: float = 0.25,
) -> dict[str, int]:
    """Fill short internal all-zero gaps for labels that reappear.

    The tracker can recover the same ID after a brief segmentation dropout, but
    CTC export cannot represent internal gaps in one track row. This fills only
    small gaps by copying the nearest boundary mask into currently empty pixels.
    Occupied frames are left alone so transient merges still become splits.
    """
    if max_gap_frames < 0:
    ...
    report = {
        ...
        "filled_gaps": 0,
        "filled_frames": 0,
        "skipped_conflict_gaps": 0,
    }
    if max_gap_frames == 0 or mask_stack.size == 0 or int(np.max(mask_stack)) == 0:
        return report

    frames_by_label = _label_frame_index(mask_stack)
    for track_id, frames in sorted(frames_by_label.items()):
        if len(frames) < 2:
            continue

        for prev_frame, next_frame in zip(frames, frames[1:]):
            gap_len = int(next_frame - prev_frame - 1)
            if gap_len <= 0 or gap_len > max_gap_frames:
                continue

            report["evaluated_gaps"] += 1
            before_mask = mask_stack[:, :, prev_frame] == track_id
            after_mask = mask_stack[:, :, next_frame] == track_id
            if not np.any(before_mask) or not np.any(after_mask):
                continue

            pending_fills: list[tuple[int, np.ndarray]] = []
            conflict = False
            for frame_idx in range(prev_frame + 1, next_frame):
                use_before = (frame_idx - prev_frame) <= (next_frame - frame_idx)
                source_mask = before_mask if use_before else after_mask
                ...
                frame = mask_stack[:, :, frame_idx]
                fill_pixels = source_mask & (frame == 0)
                fill_count = int(np.count_nonzero(fill_pixels))
                if fill_count / source_pixels < min_fill_ratio:
                    conflict = True
                    break

                pending_fills.append((frame_idx, fill_pixels))

            if conflict:
                report["skipped_conflict_gaps"] += 1
                continue

            frames_filled_for_gap = 0
            for frame_idx, fill_pixels in pending_fills:
                if np.any(fill_pixels):
                    mask_stack[:, :, frame_idx][fill_pixels] = track_id
                    frames_filled_for_gap += 1

            if frames_filled_for_gap > 0:
                report["filled_gaps"] += 1
                report["filled_frames"] += frames_filled_for_gap
...
def run_tracking(
    ...
    if gap_fill_frames < 0:
        raise ValueError("gap_fill_frames must be >= 0.")
    ...
    with manifest_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"gap_fill_frames={gap_fill_frames}\n")
    ...
    try:
        ...
        if export_mode == "final-only":
            ...
            if gap_fill_frames > 0:
                print(
                    f"[TRACKING] post: filling short internal track gaps up to {gap_fill_frames} frame(s)",
                    flush=True,
                )
                gap_fill_report = _fill_short_track_gaps(
                    mask_stack=final_tracked_tensor,
                    max_gap_frames=gap_fill_frames,
                )
                print(
                    "[TRACKING] post: gap fill "
                    f"evaluated={gap_fill_report['evaluated_gaps']} "
                    f"filled_gaps={gap_fill_report['filled_gaps']} "
                    f"filled_frames={gap_fill_report['filled_frames']} "
                    f"skipped_conflict={gap_fill_report['skipped_conflict_gaps']}",
                    flush=True,
                )
            print(
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Fill internal all-zero gaps up to N frames for labels that reappear before CTC "
            "discontinuity splitting/downsampling (default: 0 disables). Use 1-2 for brief "
            "segmentation dropouts in dense interpolated movies."
        ),
    )
